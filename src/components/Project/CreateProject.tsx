...
import { CreateProjectInput, CreateProjectResponse } from '../../types/types';
...
const CreateProject: React.FC<CreateProjectProps> = ({ onProjectCreated }) => {
    ...
    const [createProject, { loading, error }] = useMutation<CreateProjectResponse, { input: CreateProjectInput }>(CREATE_PROJECT);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const { data } = await createProject({ variables: { input: form } });
            if (data?.createProject) {
                if (onProjectCreated) onProjectCreated(data.createProject);
                setForm(initialForm);
                setSuccess(true);
            }
        } catch {
    ...
    return (
        <div className="max-w-md mx-auto bg-white rounded-xl shadow-md p-8 mt-8">
            <h2 className="text-2xl font-bold mb-6 text-center text-blue-700">Create New Project</h2>
