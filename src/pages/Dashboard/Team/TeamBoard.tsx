...
const TeamBoard = () => {
  ...
  const getTeamStats = (team: InterfaceTeam) => {
    ...
    const adminCount =
      team.users?.filter((u) => u.role?.toLowerCase() === 'admin').length || 0;
    const contributorCount =
      team.users?.filter((u) => u.role?.toLowerCase() === 'contributor')
        .length || 0;
    const viewerCount =
      team.users?.filter((u) => u.role?.toLowerCase() === 'viewer').length || 0;
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {data?.getAllTeams?.map((team: InterfaceTeam) => {
            ...
            const remainingCount =
              (team.users?.length || 0) - displayUsers.length;
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="mt-2">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getRoleBadgeColor(currentUserRole)}`}
                        >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      ...
                      {stats.contributors > 0 && (
                        <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
                          {stats.contributors} Contributor
                          {stats.contributors > 1 ? 's' : ''}
                        </span>
                      )}
                      {stats.viewers > 0 && (
